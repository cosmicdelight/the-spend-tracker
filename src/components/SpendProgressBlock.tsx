...
import { getCurrentPeriod, type SpendPeriodConfig } from "@/lib/creditCardPeriod";

/** A credit card or a bank: a recurring period plus optional min target and max cap. */
export interface SpendTargetConfig extends SpendPeriodConfig {
  spend_target: number;
  spend_cap: number | null;
}

interface Props {
  config: SpendTargetConfig;
  /** Transactions already filtered to this config's current period. */
  periodTransactions: { amount: number; personal_amount: number }[];
}

/**
 * Shared progress + status block, handling any combination of minimum spend
 * target and maximum spend cap. Used for both credit cards and banks.
 */
export default function SpendProgressBlock({ config, periodTransactions }: Props) {
  const totalCharged = periodTransactions.reduce((s, t) => s + Number(t.amount), 0);
  const personalSpend = periodTransactions.reduce((s, t) => s + Number(t.personal_amount), 0);

  const target = Number(config.spend_target) || 0;
  const cap = config.spend_cap != null ? Number(config.spend_cap) : null;
  ...
  const { daysLeft, totalDays, daysElapsed } = getCurrentPeriod(config);
